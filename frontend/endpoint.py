...
# llm = ChatOpenAI(name="gpt-4", api_key='your-api-key') # Specify your API key
...
def v2e():
    ...
    try:
        ...
        if chat_history_:
            ...
            results = cut_context([match['metadata'] for match in index.query(model.encode(str(chat_history_) + '\n' + question).tolist(), top_k=4, include_metadata=True)['matches']])
            ...
            # get top 4 results from knowledge base
            results = cut_context([match['metadata'] for match in index.query(model.encode(question).tolist(), top_k=4, include_metadata=True)['matches']])
            # get the text from the results
            source_knowledge = "\n".join([str(x) for x in results])
            # feed into an augmented prompt
...
def v2a():
    ...
    try:
        ...
        if chat_history_:
            ...
            source_knowledge = "\n".join([str(x) for x in results])
            # feed into an augmented prompt
            augmented_template = """أنت متخصص في الصحة العقلية وتساعد الأشخاص الذين يعانون من اضطرابات عقلية مثل التوحد واضطراب فرط الحركة ونقص الانتباه.\n باستخدام السياقات أدناه ، أجب على الاستعلام.
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', port=49664)
